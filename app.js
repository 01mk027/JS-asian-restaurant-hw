...
/* ADDING BUTTONS START FROM THERE */
...
buttonContainer.appendChild(btnKorea);

let btnJapan = document.createElement("button");
btnJapan.innerHTML = "Japan";
btnJapan.classList.add('btn-item');
buttonContainer.appendChild(btnJapan);

let btnChina = document.createElement("button");
btnChina.innerHTML = "China";
btnChina.classList.add('btn-item');
buttonContainer.appendChild(btnChina);
/* ADDING BUTTONS ENDS IN THERE */

btnAll.addEventListener("click",function(){
    selected = "All";
    var menuFrame = document.querySelector(".section-center");
    menuFrame.innerHTML = menu.map((item) => {
...
btnKorea.addEventListener("click",function(){
    var menuFrame = document.querySelector(".section-center");
    let koreanFoods = menu.filter(item => {
        return item.category == "Korea";
    });
    menuFrame.innerHTML = koreanFoods.map(item => {
        return `<div class="menu-items col-lg-6 col-sm-12">
        <img
          src=${item.img}
          alt=${item.title}
...
btnJapan.addEventListener("click",function(){
    ...
    menuFrame.innerHTML = japanFoods.map(item => {
        return `<div class="menu-items col-lg-6 col-sm-12">
        <img
          src=${item.img}
          alt=${item.title}
...
btnChina.addEventListener("click",function(){
    var menuFrame = document.querySelector(".section-center");
    let chineseFoods = menu.filter(item => {
        return item.category == "China";
    });
    menuFrame.innerHTML = chineseFoods.map(item => {
        return `<div class="menu-items col-lg-6 col-sm-12">
        <img
          src=${item.img}
          alt=${item.title}
